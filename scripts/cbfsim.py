...
class TelstateSubarray(Subarray):
    ...
    def target_at(self, timestamp):
        try:
            ...
            if target and target[-1][0]:    # Last element, value part of tuple
                # katpoint doesn't support bytes
                if isinstance(target[-1][0], bytes):
                    cbf_target = target[-1][0].decode()
                else:
                    cbf_target = target[-1][0]
                return katpoint.Target(cbf_target)
        except KeyError:
            pass
    ...
    def position_at(self, timestamp):
        try:
            ...
            azim = self._telstate.get_range(antenna_name + '_pos_actual_scan_azim',
                                            None, timestamp.secs)
            elev = self._telstate.get_range(antenna_name + '_pos_actual_scan_elev',
                                            None, timestamp.secs)
            if azim and elev:
...
def main():
    ...
    group.add_argument(
        '--create-fx-stream', type=str, metavar='NAME',
        help='Create a correlator stream without prompting from katcp')
    group.add_argument(
        '--create-beamformer-stream', type=str, metavar='NAME',
        help='Create a beamformer stream without prompting from katcp')
    parser.add_argument(
        '--start', action='store_true',
        help='Start the defined streams')
    parser.add_argument(
        '--dumps', type=int,
        help='Set finite number of dumps to produce for pre-configured streams [infinite]')
    parser.add_argument(
        '--cbf-channels', type=int, default=32768, metavar='N',
        help='Number of channels [%(default)s]')
    parser.add_argument(
        '--cbf-adc-sample-rate', type=float, default=1712000000.0, metavar='HZ',
        help='ADC rate [%(default)s]'),
    parser.add_argument(
        '--cbf-bandwidth', type=float, default=856000000.0, metavar='HZ',
        help='Bandwidth [%(default)s]')
    parser.add_argument(
        '--cbf-center-freq', type=float, default=1284000000.0, metavar='HZ',
        help='Sky center frequency [%(default)s]')
    parser.add_argument(
        '--cbf-spead', type=katsdptelstate.endpoint.endpoint_list_parser(7148),
        metavar='ENDPOINT', default='127.0.0.1:7148',
        help='destination for CBF output [%(default)s]')
    parser.add_argument(
        '--cbf-interface', metavar='INTERFACE',
        help='Network interface on which to send data [auto]')
    parser.add_argument(
        '--cbf-ibv', action='store_true',
        help='Use ibverbs for acceleration (requires --cbf-interface)')
    parser.add_argument(
        '--cbf-sync-time', type=float, metavar='TIME',
        help='Sync time as UNIX timestamp [now]')
    parser.add_argument(
        '--cbf-start-time', type=float, metavar='TIME',
        help='Start time of stream as UNIX timestamp [sync time]')
    parser.add_argument(
        '--cbf-int-time', type=float, metavar='TIME', default=0.5,
        help='Integration time in seconds [%(default)s]')
    parser.add_argument(
        '--cbf-substreams', type=int, metavar='N',
        help='Number of substreams (X/B-engines) in simulated CBF [auto]')
    parser.add_argument(
        '--cbf-antenna', dest='cbf_antennas', type=parse_antenna,
        action='append', default=[], metavar='DESCRIPTION',
        help='Specify an antenna (can be used multiple times)')
    parser.add_argument(
        '--cbf-antenna-file', metavar='FILE',
        help='Load antenna descriptions from file, one per line')
    parser.add_argument(
        '--cbf-antenna-names', dest='antenna_mask', metavar='NAME,...',
        help='Comma-separated list of antenna names to fake')
    parser.add_argument(
        '--cbf-sim-source', dest='cbf_sim_sources', type=parse_source,
        action='append', default=[], metavar='DESCRIPTION',
        help='Specify a source object (can be used multiple times)')
    parser.add_argument(
        '--cbf-sim-source-file', metavar='FILE',
        help='Load source descriptions from file, one per line')
    parser.add_argument(
        '--cbf-sim-clock-ratio', type=float, default=1.0, metavar='RATIO',
        help='Ratio of real time to simulated time '
             '(<1 to run faster than real time, >1 for slower)')
    parser.add_argument(
        '--cbf-sim-gain', type=float, default=1e-2, metavar='GAIN',
        help='Expected visibility for integrating 1Hz for 1s with a 1Jy source')
    parser.add_argument(
        '--cbf-target', metavar='DESCRIPTION',
        help='Set initial target')
    parser.add_argument(
        '--beamformer-timesteps', metavar='TIMES', type=int, default=256,
        help='Spectra included in each beamformer heap [%(default)s]')
    parser.add_argument(
        '--beamformer-bits', metavar='BITS', type=int, choices=[8, 16, 32], default=8,
        help='Bits per real value in beamformer data [%(default)s]')
    parser.add_argument(
        '--max-packet-size', metavar='BYTES', type=int, default=DEFAULT_MAX_PACKET_SIZE,
        help='Maximum SPEAD packet size for streams defined on command line [%(default)s]')
    parser.add_argument(
        '--servers', metavar='N', type=int, default=1,
        help='Number of servers over which the simulation is split [%(default)s]')
    parser.add_argument(
        '--server-id', metavar='N', type=int, default=1,
        help='Index of this server amongst --servers (1-based) [%(default)s]')
    parser.add_argument(
        '--port', '-p', type=int, default=7147,
        help='katcp host port [%(default)s]')
    parser.add_argument(
        '--host', '-a', type=str, default='',
